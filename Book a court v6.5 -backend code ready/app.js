...
matchDateInput.addEventListener("change", () => {
  ...
  const now = new Date();
  const isToday = matchDateInput.value === now.toISOString().split("T")[0];

  if (isToday) {
    now.setMinutes(now.getMinutes() + 5); // Add 5-minute buffer
    const hours = String(now.getHours()).padStart(2, "0");
    const minutes = String(now.getMinutes()).padStart(2, "0");
    matchTimeInput.min = `${hours}:${minutes}`;
  } else {
    matchTimeInput.removeAttribute("min");
  }
});
...
(function friendsModule() {
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const friendsData = [
      { playerId: 1, playerName: "Hassan", location: "UK", isFriend: true },
      { playerId: 3, playerName: "Nayef", location: "UK", isFriend: true },
      { playerId: 5, playerName: "Hamad", location: "UK", isFriend: true }
    ];
...
(function myBookingsModule() {
  document.addEventListener("DOMContentLoaded", () => {
    ...
    window.editMyBooking = function (id) {
      showModal(`✏️ Edit Booking #${id}`, () => {
        showModal("Edit not implemented yet.");
      });
    };
